...
describe("<Houses />", () => {
  ...
  it('Debería rederizar un "h1" con el texto "Inmobiliaria"', () => {
    expect(houses.find("h1").at(0).text()).toEqual("Inmobiliaria");
  });

  it('Debería renderizar en un tag "img" la imagen provista en el readme"', () => {
    ...
    expect(houses.find("img").at(0).prop("src")).toEqual('https://blog.wearedrew.co/hubfs/claves%20para%20una%20venta%20inmobiliaria%20exitosa.jpg');
  });
  ...
  describe("React LifeCycles", () => {
    ...
    it("Debería pasar como props a cada componente <HouseCard /> las propiedades id, region, name, description y price de cada house", () => {
      // PASARLE LA PROP keys en el mapeo.
      expect(houses.find(HouseCard).at(0).props().id).toEqual(1);
      expect(houses.find(HouseCard).at(0).props().region).toEqual("Norte");
      expect(houses.find(HouseCard).at(0).props().name).toEqual("Departamento 1");
      expect(houses.find(HouseCard).at(0).props().price).toEqual(
        "1500"
      );
      expect(houses.find(HouseCard).at(1).props().id).toEqual(2);
      expect(houses.find(HouseCard).at(1).props().price).toEqual(
        "1800"
      );
